...
localparam EXECUTION   		      = 4'd4;
localparam MEMORY_ACCESS          = 4'd5;
localparam WRITEBACK              = 4'd6;
localparam PC_GENERATION          = 4'd7;
localparam END     			      = 4'd8;
...
assign ALUPCRes_i_w = curr_pc[30:0] + (imm_r<<1);
...
always @ (*) begin
	if( curr_state == PC_GENERATION) begin
		if(b_type_w & ALUcond_w) 
...
always @ (posedge i_clk or negedge i_rst_n) begin
	if(!i_rst_n) begin
		...
		o_status_r <=  curr_state >= DECODE ?  o_status_w:0;
